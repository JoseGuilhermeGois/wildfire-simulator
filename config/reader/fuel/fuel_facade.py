...
from config.reader.fuel.fuel import Fuel
...
class BaseFuelFacade(FuelFacade):
    ...
    def get_fuel_models(self, fuel_model_name: str, fuel_model_characteristics: list[float]) -> Fuel:
        ...
        flame_length = self.to_flame_length(fuel_flame_length)
    ...
    def to_heat_content(self, heat_content_dead: float, heat_content_alive: float):
    ...
    def to_bed_depth(self, bed_depth: float) -> float:
        return float(bed_depth * BED_DEPTH_CONSTANT)

    def to_flame_length(self, flame_length: float) -> float:
        return float(flame_length * FLAME_LENGTH_CONSTANT)

    def to_moisture_extinction_dead(self, moisture_extinction_dead: float) -> float:
